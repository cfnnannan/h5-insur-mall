...
<div class="insure-contaion">
  <div class="home-insure" id="get-focus">
    <div class="insure-title padding">
      ...
      <div class="right">
        ...
        :key="item.id"
        :class="{'actived': selectedRel==item.id }"
    ...
    <div class="insure-msg">
      <div class="suffer-msg" v-show="selectedRel!='01'">
      ...
      <div class="judge-msg">
        ...
        <div class="right">
          <div class="select-btn" 
            v-for="item of isMarried"
            :key="item.key"
            @click="chooseMarry(item.key)"
            :class="{'actived': selectedMarry==item.key}">{{item.name}}
          </div>
        </div>
      </div>
      <div class="msg-cont">
        <div class="msg judge-msg">
          <label>孕周</label>
          <div class="right">
            <select dir="rtl" v-model="getWeekData" @blur="closeFocus">
              <option 
                  v-for="(item,index) of marriedWeek" 
                  :key="index" 
                  :value="index">{{item}}</option>
            </select>
            <span class="iconfont" style="color: #333">&#xe6aa;</span>
          </div>
        </div>
        <div class="msg judge-msg">
          <label>孕天</label>
          <div class="right">
            <select dir="rtl" v-model="getDay" @blur="closeFocus">
              <option 
                  v-for="(item,index) of marrieDay" 
                  :key="index"
                  :value="index">{{item}}</option>
            </select>
            <span class="iconfont" style="color: #333">&#xe6aa;</span>
  ...
  <yd-popup v-model="showClause" position="center" width="92%">
    <popclause :afferentClauseData="'wInsure'" @closeClause="showClause=false"></popclause>
  </yd-popup>
  <!-- 投保须知弹框 -->
  <yd-popup v-model="showNotice" position="center" width="92%">
    <popnotice :afferentNoticeData="'wInsure'" @closeNotice="showNotice=false"></popnotice>
  </yd-popup>
  <yd-popup v-model="inforNotice" position="center" width="92%">
    <div class="model-inform model-rate">
        ...
        <div class="textarea" v-show="selectedTab==0">
          ...
          11、被保险人过去没有被保险公司解除合同或投保、复效时被拒保、延期的情况。
        </div>
        <div class="textarea" v-show="selectedTab==1">
          ...
          <div>
            ...
            <br>3、	本人<span v-show="selectedRel!='01'">及被保险人（本人承诺已取得被保险人代为处理其个人信息的全权委托）</span>同意贵司以提供本保险或与此相关的服务为目的，而收集或使用本人<span v-show="selectedRel!='01'">及被保险人</span>的个人信息，无论该信息是从本次投保申请或其他渠道所获取。本人<span v-show="selectedRel!='01'">及被保险人</span>理解并同意授权贵司及贵司合作伙伴为提升服务质量而可能查询、提供或使用本人<span v-show="selectedRel!='01'">及被保险人</span>个人信息。本人<span v-show="selectedRel!='01'">及被保险人</span>理解此声明自投保时生效，具有独立法律效力，不受保险合同效力状态变化的影响。
          </div>
        </div>
      <div class="confirm">
        ...
        <div class="handel" @click="getYcwyOrder">
          <span class="iconfont">&#xe62a;</span>
          <span>全部符合并支付</span>
        </div>
      </div>
...
export default {
  ...
  data () {
    return {
      ...
      checkedValue: false,
      errorMsg: '',
      marriedWeek: ['0周','1周','2周','3周','4周','5周','6周',
      '7周','8周','9周','10周','11周','12周','13周','14周','15周',
      '16周','17周','18周','19周','20周'],
      marrieDay: ['1天','2天','3天','4天','5天','6天','7天',],
      isMarried:  [
        {key: '是', name: '已婚'},
        {key: '否', name: '未婚'}
        ],
  ...
  mounted() {
    ...
    if (getParams.getURLParams(URL).source == 'mall') {
      this.redirectUrl = getEnv().redirectUrl+'/#/payment'+'?source=mall&product=ycwy&payState=success'
    } else {
      this.redirectUrl = getEnv().redirectUrl+'/#/payment'+'?source=self&product=ycwy&payState=success'
    }
  ...
  methods: {
    ...
    submit() {
      ...
      if(this.selectedRel=='01') {
        this.insurerName = this.holderName;
        this.insurerId = this.holderId;
        if(checkUtil.commonCheck.checkAge(this.currentDate, this.holderId, 20, 40)) {
          this.popError('本产品只限20周岁（含）-40周岁（含）女性购买');
          this.errorIcon.hIdIcon=true;
          return;
        } this.errorIcon.hIdIcon=false
      } else {
      ...
      if(checkUtil.commonCheck.checkAge(this.currentDate, this.insurerId, 20, 40))  {
        this.popError('此产品只限20周岁（含）-40周岁（含）女性投保');
        this.errorIcon.iIdIcon=true
        return;
      ...
      this.holderBirthDate = this.holderId.substring(6,10)+"-"+ this.holderId.substring(10,12)+"-"+this.holderId.substring(12,14)
      this.insurerGender = '0'+ checkUtil.commonCheck.judgeSexRel(this.insurerId);
      if(this.insurerGender == '01') {
        this.popError('此产品只限于女性投保');
        return;
      }
      if(this.selectedMarry == '否') {
        this.popError('此产品只限于已婚女性投保');
        return;
      }
      this.insurerBirthDate = this.insurerId.substring(6,10)+"-"+ this.insurerId.substring(10,12)+"-"+this.insurerId.substring(12,14)
    ...
    getYcwyOrder() {
      ...
      this.$store.dispatch('getEanOrder', {
        data: {
          ...
          insurerBirthDate: this.insurerBirthDate,
